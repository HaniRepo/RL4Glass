# -*- coding: utf-8 -*-
"""DisturbParametersAir.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Psrvxp1XK9zybjwjK4jSktUSnNEmKUQB
"""
...
file_path_perturbations_air = '/content/simulation_perturbations_air_log.csv'

# Define perturbation scenarios for T_air
perturbations_air = [
    {"label": "Baseline", "T_air_initial": 50},
    {"label": "+5 Initial T_air", "T_air_initial": 55},
    {"label": "-5 Initial T_air", "T_air_initial": 45},
]

# Open the CSV file to log the data
with open(file_path_perturbations_air, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow([
    ...
    for perturbation in perturbations_air:
        label = perturbation["label"]
        T_air_initial = perturbation["T_air_initial"]
        T_gob = 1100  # Fixed for this experiment

        # Reset simulator with steady-state conditions
        simulator.reset(T_mold=steady_mold_temp)
        simulator.T_gob = T_gob  # Nominal T_gob
        simulator.T_air = T_air_initial  # Adjusted initial T_air
        simulator.t_settle = degrees_to_seconds(PROPORTION_SETTLE * CONTACT_NOMINAL_DEG)
        simulator.t_counter = degrees_to_seconds(PROPORTION_COUNTER * CONTACT_NOMINAL_DEG)
        simulator.t_cooling = degrees_to_seconds(COOLING_NOMINAL_DEG)
        ...
        initial_air_temps = [T_air_initial]  # Record initial T_air for each cycle
        ...
        for i in range(1, 101):  # Simulate 100 cycles
            ...
            # Log data
            writer.writerow([label, i, time_step, parison_temp, mold_temp, T_gob, T_air_initial])

            # Append the results to lists for plotting
            parison_temps.append(parison_temp)
            mold_temps.append(mold_temp)
            time_steps.append(time_step)
            initial_air_temps.append(T_air_initial)
        ...
        results_air[label] = {
            ...
            "initial_air_temp": T_air_initial,
        }

# Plot Parison Temperatures
plt.figure(figsize=(10, 6))
for label, data in results_air.items():
    plt.plot(
        data["time"], data["parison_temp"],
        label=f"{label} (Initial T_air: {data['initial_air_temp']}°C)"
    )
plt.xlabel("Simulation Time (s)")
plt.ylabel("Parison Temperature (°C)")
plt.title("Effect of Initial T_air Perturbations on Parison Temperature")
plt.legend()
plt.grid(True)
plt.show()
...
plt.title("Effect of Initial T_air Perturbations on Mold Temperature")
plt.legend()
plt.grid(True)
plt.show()

# Download the CSV log file
print(f"\nResults have been saved in the CSV file: {file_path_perturbations_air}")
files.download(file_path_perturbations_air)